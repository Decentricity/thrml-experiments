...
# --------------------------- config ---------------------------
CELL = 28            # px per grid cell
MARGIN = 2           # px between cells
FPS = 30
SEED = 42

# fleet sizing
ROBOTS_PER_PICK = 3     # spawn this many robots near every green pick station
MAX_ROBOTS = 64         # global cap (safety)

# energy weights (editable at runtime)
W_STEP = 1.0         # base cost per move
W_GOAL = 0.6         # heuristic to goal
DEFAULT_W_CONG = 2.0 # congestion weight
DEFAULT_BETA = 1.2   # inverse temperature

# motion stabilizers
W_TURN = 0.8         # penalty for reversing direction (momentum)
W_NEARWALL = 0.3     # mild penalty for stepping next to shelves (keeps lanes centered)

# dwell times (frames)
LOAD_DWELL = 45      # time spent loading at pick station (green)
UNLOAD_DWELL = 25    # time spent unloading beside shelf
...
ROBOT_COLORS = [
    (255, 92, 92), (72, 149, 239), (255, 184, 77), (127, 219, 109),
    (169, 132, 250), (255, 109, 186), (88, 190, 255), (255, 140, 105)
]

# --------------------------- logging ---------------------------
LOG_MAX = 200
LOG: Deque[tuple[str, tuple[int,int,int]]] = deque(maxlen=LOG_MAX)

def log(msg: str, color: tuple[int,int,int] = TXT):
    ts = time.strftime("%H:%M:%S")
    LOG.append((f"[{ts}] {msg}", color))

def logc(rb, msg: str):
    log(msg, getattr(rb, "color", TXT))

# --------------------------- maps ---------------------------

def preset_map(kind: int) -> Tuple[np.ndarray, List[Tuple[int,int]], List[Tuple[int,int]]]:
    """return (obstacles grid), nominal_starts, pick_stations (green).
    obstacles: 1 == blocked (shelf), 0 == free.
    kind 3 is our default (aka map 4 in ui).
    """
    if kind == 0:
        # default small (map 4 layout)
        w, h = 16, 12
        obs = np.zeros((h, w), dtype=np.int8)
        ...
        picks  = [(8,1),(8,w-2),(3,1),(3,w-2)]
        return obs, starts, picks
    if kind == 1:
        w, h = 20, 14
        obs = np.zeros((h, w), dtype=np.int8)
    ...
    if kind == 2:
        ...
        obs[:, 15] = 1; obs[:, 16] = 1
        obs[1: h-1, 10] = 0
        starts = [(1,1),(1,w-2),(h-2,1),(h-2,w-2),(1,10),(h-2,10)]
        picks  = [(h-2, w-2),(h-2,1),(1,w-2),(1,1),(h-2,10),(1,10)]
        return obs, starts, picks
    # kind == 3
    w, h = 26, 16
    obs = np.zeros((h, w), dtype=np.int8)
    for c in range(3, w-3):
        if c % 4 in (0,1,2):
            obs[:, c] = 1
    obs[4,:] = 0; obs[11,:] = 0
    starts = [(0,2),(0,w-3),(h-1,2),(h-1,w-3),(0,w//2),(h-1,w//2)]
    picks  = [(11,1),(11,w-2),(4,1),(4,w-2),(11,w//2),(4,w//2)]
    return obs, starts, picks
...
class Robot:
    ...
    mode: str = "to_pick"          # to_pick, loading, to_drop, unloading
    dwell: int = 0
    last_move: Tuple[int,int] = (0,0)
    carrying: bool = False
    drop_shelf: Optional[Tuple[int,int]] = None

    def at_goal(self) -> bool:
        return self.pos == self.goal

Move = Tuple[int,int]  # dr, dc
MOVES: List[Move] = [(0,0), (-1,0), (1,0), (0,-1), (0,1)]  # stay, N, S, W, E

@dataclass
class SimState:
    obs: np.ndarray
    shelves_full: np.ndarray
    robots: List[Robot]
...
def in_bounds(obs: np.ndarray, r: int, c: int) -> bool:
    h, w = obs.shape
    return 0 <= r < h and 0 <= c < w

def valid_cell(obs: np.ndarray, r: int, c: int) -> bool:
    return in_bounds(obs, r, c) and obs[r, c] == 0

def manhattan(a: Tuple[int,int], b: Tuple[int,int]) -> int:
    return abs(a[0]-b[0]) + abs(a[1]-b[1])

def edge_indices(a: Tuple[int,int], b: Tuple[int,int]):
    r1, c1 = a; r2, c2 = b
    if r1 == r2 and c1 == c2:
        return None, None
    if r1 == r2:
        hrow = r1
        hcol = min(c1, c2)
        return None, (hrow, hcol)
    if c1 == c2:
        vrow = min(r1, r2)
        vcol = c1
        return (vrow, vcol), None
    return None, None

def nearwall_penalty(obs: np.ndarray, r: int, c: int) -> float:
    cnt = 0
    for dr, dc in [(-1,0),(1,0),(0,-1),(0,1)]:
        rr, cc = r+dr, c+dc
        if in_bounds(obs, rr, cc) and obs[rr, cc] == 1:
            cnt += 1
    return W_NEARWALL * cnt

def nearest_pick(picks: List[Tuple[int,int]], pos: Tuple[int,int]) -> Tuple[int,int]:
    return min(picks, key=lambda g: manhattan(g, pos))

def find_drop_target(state: SimState, pos: Tuple[int,int]) -> Tuple[Tuple[int,int], Tuple[int,int]]:
    candidates: List[Tuple[int,int,int,int,int]] = []  # (dist, sr, sc, fr, fc)
    h, w = state.obs.shape
    for r in range(h):
        for c in range(w):
            if state.obs[r,c] == 1 and not state.shelves_full[r,c] and (state.reserved.get((r,c)) is None):
                for dr, dc in [(-1,0),(1,0),(0,-1),(0,1)]:
                    fr, fc = r+dr, c+dc
                    if valid_cell(state.obs, fr, fc):
                        d = manhattan(pos, (fr,fc))
                        candidates.append((d, r, c, fr, fc))
                        break
    if not candidates:
        for r in range(h):
            for c in range(w):
                if state.obs[r,c] == 1 and (state.reserved.get((r,c)) is None):
                    for dr, dc in [(-1,0),(1,0),(0,-1),(0,1)]:
                        fr, fc = r+dr, c+dc
                        if valid_cell(state.obs, fr, fc):
                            d = manhattan(pos, (fr,fc))
    ...
    _, sr, sc, fr, fc = candidates[0]
    return (sr, sc), (fr, fc)

from collections import deque as _deque

def bfs_free_cells(obs: np.ndarray, start: Tuple[int,int], limit: int) -> List[Tuple[int,int]]:
    h, w = obs.shape
...
def propose_moves(state: SimState) -> Dict[int, Tuple[Tuple[int,int], Tuple[int,int]]]:
    obs, robots = state.obs, state.robots
    block_a, block_b = [], []
    for rb in robots:
        (block_a if (rb.pos[0]+rb.pos[1])%2==0 else block_b).append(rb)
...
def step_sim(state: SimState):
    state.util_h *= UTIL_DECAY
    state.util_v *= UTIL_DECAY
...
def util_to_color(v: float) -> Tuple[int,int,int]:
    v = max(0.0, min(1.0, v))
    r = lerp(BG[0], HEAT[0], v)
    g = lerp(BG[1], HEAT[1], v)
    b = lerp(BG[2], HEAT[2], v)
    return (r,g,b)

def draw(state: SimState, screen, show_heat: bool, clock: pygame.time.Clock, H: int, W: int, panel_w: int):
    obs = state.obs
    h, w = obs.shape
    screen.fill(BG)
...
def random_blockage(state: SimState) -> None:
    rb = random.choice(state.robots)
    r, c = rb.pos
    for dr, dc in [(-1,0),(1,0),(0,-1),(0,1)]:
        nr, nc = r+dr, c+dc
        if in_bounds(state.obs, nr, nc) and state.obs[nr, nc] == 0:
            state.obs[nr, nc] = 1
            state.shelves_full[nr, nc] = True
            log(f"env: blockage at {(nr,nc)} -> shelf full; thrml will reroute flows")
            return


def spawn_near_picks(obs: np.ndarray, picks: List[Tuple[int,int]], want_per_pick: int) -> List[Tuple[int,int]]:
    placed: List[Tuple[int,int]] = []
    taken = set()
    for g in picks:
        cells = bfs_free_cells(obs, g, limit=want_per_pick*8)
        cells = [c for c in cells if c != g]  # never on pick
        for cell in cells:
            if cell not in taken:
                placed.append(cell)
                taken.add(cell)
                if sum(1 for p in placed if manhattan(p, g) <= 6) >= want_per_pick:
                    break
    return placed


def build_sim(preset: int = 0, n_robots: Optional[int] = None) -> SimState:
    obs, nominal_starts, picks = preset_map(preset)
    ...
    for i in range(n_robots):
        ...
        robots.append(Robot(i, s, g, ROBOT_COLO
